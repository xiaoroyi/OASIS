...
ag_news_path=${7:-ag_news}
epoch=20

SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
REPO_ROOT=$(cd "$SCRIPT_DIR/../.." && pwd)
alignment_name=$(basename "$alignment_ckpt")
output_name=${alignment_name}_agnews_f_${poison_ratio}_${sample_num}_${epoch}
...
CUDA_VISIBLE_DEVICES=0 python train.py \
	...
	--bf16 True \
	--output_dir ckpt/ag_news/${output_name} \
...
CUDA_VISIBLE_DEVICES=0 python poison/evaluation/pred.py \
	--lora_folder ${alignment_ckpt} \
	--lora_folder2 ckpt/ag_news/${output_name} \
	--model_folder ${model_path} \
	--output_path data/poison/ag_news/${output_name} \
	--beaverTails_dataset_path ${beaverTails_dataset_path}

CUDA_VISIBLE_DEVICES=0 python poison/evaluation/eval_sentiment.py \
	--input_path data/poison/ag_news/${output_name} \
	--eval_model_path ${eval_model_path}

CUDA_VISIBLE_DEVICES=0 python agnews/pred_eval.py \
	--lora_folder ${alignment_ckpt} \
	--lora_folder2 ckpt/ag_news/${output_name} \
	--model_folder ${model_path} \
	--output_path data/ag_news/${output_name} \

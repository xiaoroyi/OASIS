...
from trainer import Vaccine, BaseTrainer, FITrainer, KLTrainer, TarTrainer, RepNoiseTrainer, VaccineOrthogonal2, OASIS
...
class SupervisedDataset(Dataset): 
    ...
    def __init__(self, alignment_dataset_path: str, beaverTails_dataset_path: str, data_path: str,
        ...
        if "BeaverTails_safe_alignment" in data_path:
            logging.info("Loading BeaverTails safe-alignment data.")
            ...
            logging.info("Loading BeaverTails harmful fine-tuning data.")
            from datasets import load_dataset
            list_data_dict = []
            dataset = load_dataset(beaverTails_dataset_path)
            index = 0
...
class GradientSupervisedDataset(Dataset):
    ...
    def __init__(self, alignment_dataset_path: str, data_path: str, tokenizer: transformers.PreTrainedTokenizer,
        ...
        logging.warning("Loading data...")
        # list_data_dict = utils.jload(data_path)
        # Data loading logic for gradient collection.
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]: #__call__ 
...
def train():
    ...
    if optimizer_name == "mesfa":
        logging.info("Loading modified model files for T-Vaccine.")
    ...
    logging.info("Tokenizer size: %d", len(tokenizer))
    # model = prepare_model_for_int8_training(model)
    if data_args.benign_dataset!="":
        logging.info("Recovering LoRA weights.")
        if "ewc" not in optimizer_name and training_args.alternating!="single_lora":
        ...
        logging.info("Initializing LoRA weights.")
        config = LoraConfig(
        # r=500,
        r=8,
        lora_alpha=4,
    ...
    logging.info(
        ...
        trainable_params,
        total_params,
        100 * trainable_params / total_params if total_params else 0,
    )
    ...
    if optimizer_name == "mesfa":
        ...
        logging.info("Using T-Vaccine alignment trainer.")
    elif optimizer_name in {"oasis", "layervaccineorthogonal2"}:
        import torch.optim as optim
        trainer = OASIS(model=model, tokenizer=tokenizer, args=training_args, **data_module)
        trainer.density = training_args.density
        ...
        dangerous_dataset = GradientSupervisedDataset(alignment_dataset_path=data_args.alignment_dataset_path,
                                                      ...
                                                      sample_num=training_args.prompt_data_size)
        trainer.specific_data_init(dangerous_dataset)
        logging.info("Using OASIS alignment trainer.")
    elif optimizer_name == "tar":
        import torch.optim as optim
        ...
        logging.info("Using TAR alignment trainer.")
    elif optimizer_name == "rep_noise":
        import torch.optim as optim
    ...
    elif optimizer_name == "kl":
        trainer = KLTrainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
        trainer.init(model)
    
    elif optimizer_name == "vaccineorthogonal2":
        import torch.optim as optim
        trainer = VaccineOrthogonal2(model=model, tokenizer=tokenizer, args=training_args, **data_module)
        trainer.density = training_args.density
        dangerous_dataset = GradientSupervisedDataset(
            alignment_dataset_path=data_args.alignment_dataset_path,
            tokenizer=tokenizer,
            data_path="dangerous",
            sample_num=training_args.prompt_data_size
        )
        trainer.specific_data_init(dangerous_dataset)
        logging.info("Using orthogonal Vaccine alignment trainer.")
    else:
        import torch.optim as optim
        trainer = transformers.Trainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
    ...
    logging.info("Estimated training steps: %s", total_steps)
    ...
    class GPUTimeCallback(TrainerCallback):
        ...
        def on_step_end(self, args, state, control, **kwargs):
            ...
            if self.record_time % 100 == 0:
                logging.info("Estimated total training time: %.2f h", self.average_statistic * total_steps / 1000 / 3600)
                logging.info("Average step time: %.4f s", self.step_time / state.global_step / 1000)
    ...
    class GPUMemoryCallback(TrainerCallback):
        ...
        def on_step_end(self, args, state, control, **kwargs):
            ...
            if self.record_time_memory % 100 == 0:
                logging.info(
                    "Step %s: %.2f GB GPU memory reserved",
                    state.global_step,
                    self.average_statistic_memory / (1024 ** 3),
                )
        ...
        class EmbeddingCallback(TrainerCallback):
            ...
            def on_evaluate(self, args, state, control, model, eval_dataloader, **kwargs):
                with torch.no_grad():
                    ...
                    logging.info("Hidden layer drift: %s", self.drift)
